...
async function addLink(body, adminUsername) {
  const url = normalizeUrl(body.url);
  if (!url) throw new Error('A valid URL is required');
  const host = new URL(url).hostname;

  const rawHosting = body.hostingType ? String(body.hostingType).toLowerCase().trim() : '';
  const hostingType = HOSTING_TYPES.includes(rawHosting) ? rawHosting : heuristicHosting(host);
  const strictness = body.strictness !== undefined ? clampInt(body.strictness, 1, 10, 4) : null;
  const blockers = coerceBlockers(body.blockers);
  const rawName = String(body.name || '').trim();
  const name = (rawName || deriveName(url)).slice(0, 120);
  const now = Date.now();
  const item = {
    ...
    name,
    hostingType,
    strictness: strictness == null ? undefined : strictness,
    strictnessMin: 1,
    strictnessMax: 10,
    blockers,
    status: 'active',
...
async function updateLink(body) {
  ...
  if (body.name !== undefined) {
    setExpr.push('#n = :n');
    names['#n'] = 'name';
    const derived = body.url !== undefined ? deriveName(normalizeUrl(body.url) || existing.Item.url) : existing.Item.name;
    values[':n'] = (String(body.name || '').trim().slice(0, 120)) || derived;
  }
...
export async function handler(event) {
  ...
  try {
    ...
    switch (method) {
      ...
      case 'POST':
        return respond(201, await addLink(body, auth.username), origin);
      case 'PUT':
    ...
    const status = /required|not found|no fields/i.test(err.message) ? 400 : 500;
