...
try {
  ...
  aws lambda update-function-configuration --function-name $functionName --runtime nodejs20.x --handler index.handler --timeout 30 --environment $envVars --region $region --no-cli-pager | Out-Null
  aws lambda wait function-updated --function-name $functionName --region $region
